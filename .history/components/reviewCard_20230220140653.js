...
reviews.map( (rev, index) => {
            return (
              <div key={index} style={{display:'flex', flexDirection:'column', height:'100%', padding:10}}>
            ...
            <div style={{lineHeight:1.2,paddingLeft:25, color:'#184B4B'}}>
                <h2 style={{fontFamily:'righteous, cursive'}}>{rev.name}</h2>
                <h3 style={{fontFamily:'righteous, cursive'}}>{rev.rating}</h3>
                <p>{rev.review}</p>
            </div>
        </div>
            )
