...
test.describe('testing for home Desktop', () => {
    test("Testing for number of columns", async ({ page }) => {
        await page.goto('http://localhost:3000')

        const revDiv = page.locator('#flex');

        const checkingWrap = await revDiv.evaluate((ele) => {
            return window.getComputedStyle(ele).getPropertyValue('flex-wrap')
        })

        console.log(checkingWrap);
        expect(checkingWrap).toBe("wrap");
    })
})
